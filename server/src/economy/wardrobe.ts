 ...
 * ## Economy
 *
 * TICKETS only, always through the ledger, never a direct balance mutation -
 * repo-root CLAUDE.md's rule, unchanged by this feature. Gold Coins buy
 * plays; Tickets buy looks. Nothing here reads or writes GC.
 *
 * The debit uses SKIN_PURCHASE_TICKETS rather than a new enum value, which
 * is the same call itemShop.ts already made and documented: that type's own
 ...
 * also keeps the ledger's hard invariant literally true (TICKETS have
 * exactly one legal credit path and one legal debit path) instead of
 * quietly widening it, and avoids an enum migration. Which piece and slot
 * was bought is recorded in the transaction's `meta` for audit, same as
 * items record itemId/itemCategory.
 *
...
export type PurchasePieceOutcome =
  ...
  | { ok: false; reason: "INSUFFICIENT_TICKETS"; piece: WardrobePieceDef; balanceTickets: number };

/**
 * Buys wardrobe piece `id` with TICKETS and wears it immediately, in one DB
...
export async function purchasePiece(
  ...
  if (balanceTickets < piece.price) {
    return { ok: false, reason: "INSUFFICIENT_TICKETS", piece, balanceTickets };
  }

  const transaction = await applyTransaction(tx, userId, "TICKETS", "SKIN_PURCHASE_TICKETS", -piece.price, {
