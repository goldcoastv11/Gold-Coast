/**
 * Player Room furniture catalog (roadmap/room-furniture) - the third decor
 * category alongside src/roomCatalog.ts's wallpaper/flooring, deliberately
 * kept in its own file rather than folded into that one - see this file's
 * "Why this is its own file" section.
 *
 ...
 *     always also equips; server's economy/furniture.ts purchasePiece
 *     never does) - see that file's header for why.
 ...
 *
 * No art is sourced - per the founder's direction, matching how
 * BootScene.ts already draws the room's wallpaper/flooring procedurally.
 * Each piece's `id` doubles as its texture key; BootScene.ts's
 * createFurnitureTextures() registers a texture under every id in
 * FURNITURE_CATALOG.
 *
 * ## Duplication with the server copy is deliberate
 *
 * Same reasoning as roomCatalog.ts's own header: no shared package to
 * import from, so this file and server/src/furnitureCatalog.ts are kept in
 * sync by hand. Keep ids/prices/slot ids identical - the server copy is
 * what's actually charged and persisted; this one is what's shown and
 * rendered.
 */

/**
 * A fixed placement position in the Player Room. Stable, named identity
 * (not an array index) - matches the server's FurnitureSlot enum exactly,
 * so adding a slot later can never silently move an existing placement.
 */
 ...
 * spawn-to-door walking corridor. Each id here maps to a world pixel
 * position in src/scenes/RoomScene.ts's FURNITURE_SLOT_POSITIONS. Every
 * slot here is emptyable - there is no `optional` field the way
 * roomCatalog.ts's RoomSlotDef has one, because for furniture that would
 * always read `true`.
 */
...
export interface FurniturePieceDef {
  ...
  price: number;
  /** Swatch tint for the shop panel, used before/instead of the real drawn texture in a small UI thumbnail. */
...
export function getFurnitureSlotDef(slot: FurnitureSlotId): FurnitureSlotDef | undefined {
...
/**
 * What the room's furniture currently looks like: one entry per OCCUPIED
 * slot only. Unlike roomCatalog.ts's EquippedRoom (always fully populated
 * via a free-default fallback), a missing key here means genuinely empty,
 * not "fell back to something" - there is nothing to fall back to.
 */
export type PlacedFurniture = Partial<Record<FurnitureSlotId, string>>;
