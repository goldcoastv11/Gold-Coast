...
describe("ledger", () => {
  it("credits and debits move the balance and record a transaction", () => {
    const ledger = createLedger(100);
    applyTransaction(ledger, "GC", "ADJUST_GC", 50);
    expect(getBalance(ledger, "GC")).toBe(150);

    applyTransaction(ledger, "GC", "ADJUST_GC", -5);
    expect(getBalance(ledger, "GC")).toBe(145);
    expect(ledger.transactions).toHaveLength(2);
    expect(ledger.transactions[1].balanceAfter).toBe(145);
  });
...
describe("GC packages - plain GC top-up, one currency", () => {
  it("purchasePackage grants exactly the package's GC amount", () => {
...
describe("signup bonus - GC only", () => {
  it("grants GC (default 1x)", () => {
    const ledger = createLedger(0);
...
/**
 * The "ad rewards - GC only, never TICKETS" block that used to live here
 * covered src/economy/adRewards.ts (the local-ledger claimAdRewardGc
 * wrapper). Removed along with that module (2026-08-30
 * roadmap/deadcode) - it had zero real callers (no scene ever called
 * `gameState.claimAdReward()`/`claimBonus()`; the overworld's Coin Kiosk
 * uses the separate, still-live claimAttendantBonus path in
 * attendantClaim.ts, covered by attendantClaim.test.ts).
 *
 ...
 * in server/test/wardrobe.test.ts. The pure layering rules that DO still
 * live on the client are covered in src/wardrobeCatalog.test.ts.
 */
